...
<body>
    <div id="app">
        ...
        <!-- 메인 영역 -->
        <section class="admin_specialty grid">
            <?php foreach($list as $item): ?>
                <div class="item">
                    <div class="photo">
                        <img src="./(웹디자인)선수제공파일/특산품/<?=$item->img ?>" alt="specialtyImg" title="specialtyImg">
                    </div>
                    <div class="text">
                        <h4><?=$item->area ?></h4>
                        <p>특산품: <?=str_replace(",", ", ", $item->specialtys) ?></p>
                    </div>
                    <button class="btn"><a href="/detail?area=<?=$item->area ?>">관리</a></button>
                </div>
            <?php endforeach; ?>
        </section>
        <!-- 푸터 영역 -->
